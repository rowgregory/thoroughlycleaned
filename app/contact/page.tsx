...
const Contact = () => {
  const { isLoading, data } = useFetchPageSpecificTextBlocksQuery(['CONTACT_PAGE', 'HEADER', 'FOOTER'])
      ...
      <section className="px-4 990:px-12 xl:px-4 bg-white">
        <div className="max-w-[520px] 760:max-w-[700px] 990:max-w-[960px] 1200:max-w-[1280px] mx-auto w-full py-32 grid grid-cols-12 gap-y-16 md:gap-x-8 items-center relative">
          <div className="col-span-12 990:col-span-6">
            <SectionHeader
              subtitle={data?.transformedTextBlocks?.CONTACT_PAGE?.contactPageSubtitle}
              subtitleName="contactPageSubtitle"
              title={data?.transformedTextBlocks?.CONTACT_PAGE?.contactPageTitle}
              titleName="contactPageTitle"
              type="CONTACT_PAGE"
            />
            <EditableTextArea
              tag="p"
              initialValue={data?.transformedTextBlocks?.CONTACT_PAGE?.contactPageParagraph}
              type="CONTACT_PAGE"
              textBlockKey="contactPageParagraph"
              className="text-espresso"
            />
            <div className="flex flex-col gap-y-10 mt-10">
              {navigationDrawerData(data?.transformedTextBlocks).map((item, i) => (
                <div key={i} className="grid grid-cols-12 group gap-x-3">
                  <div className="col-span-3 480:col-span-2 bg-skyAqua rounded-full w-14 h-14 flex items-center justify-center">
                    <AwesomeIcon icon={item.icon} className="w-6 h-6 text-sunny" />
                  </div>
