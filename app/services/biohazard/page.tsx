...
const Biohazard = () => {
  ...
  const { isLoading: loadingTextBlocks, data } = useFetchPageSpecificTextBlocksQuery(['SERVICE_BIOHAZARD_PAGE'])
  const textBlockMap = data?.transformedTextBlocks
      ...
      <div className="px-4 lg:px-12 xl:px-4 py-32 relative">
        <div className="max-w-2xl 990:max-w-screen-xl mx-auto w-full grid grid-cols-12 gap-y-10 990:gap-x-10">
          <div className="order-2 990:order-1 flex items-center justify-center col-span-12 990:col-span-6">
            <EditableImage
              ...
              className="w-full h-full object-cover aspect-video 990:aspect-square"
              priority={false}
            />
          </div>
        ...
        <div className="max-w-2xl 990:max-w-screen-xl mx-auto w-full grid grid-cols-12 gap-y-10 sm:gap-10 mt-14">
          {loadingServices
            ...
            : services?.services?.map((service: Service) => <ServiceCard key={service?.id} {...service} />)}
        </div>
      </div>
    </>
  )
