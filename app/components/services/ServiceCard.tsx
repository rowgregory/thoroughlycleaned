...
const ServiceCard: FC<ServiceCardProps> = ({ url, name, description, serviceType }) => {
  ...
  return (
    <div
      ...
      className="col-span-12 760:col-span-6 990:col-span-4 shadow-serviceCard group cursor-pointer"
    >
      <div
        style={{ backgroundImage: `url(${url})` }}
        className="bg-paleBlue bg-cover bg-no-repeat bg-center w-full h-auto aspect-[10/7] relative"
      >
        <div className="absolute z-10 bottom-2 right-2 bg-sunny rounded-lg w-20 h-auto aspect-square flex items-center justify-center transform transition-all group-hover:-translate-y-2 ease-in-out duration-500">
      ...
      <div className="bg-white p-6 relative flex flex-col">
        ...
        <div className="flex items-center gap-x-2 mt-10">
          ...
          <div className="w-6 h-6 rounded-full bg-sunny flex items-center justify-center group-hover:translate-x-2 duration-500 ease-in-out transform transition-all">
            <AwesomeIcon icon={arrowRightIcon} className="w-3 h-3" />
          </div>
        </div>
      </div>
    </div>
