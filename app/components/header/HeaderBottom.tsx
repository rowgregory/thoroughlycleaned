...
const HeaderNavLink = ({ link }: any) => (
  <div className="relative group h-full flex items-center justify-end">
    <Link
      href={link.linkKey}
      className={`${link.active ? 'text-skyAqua' : 'text-zinc-800'} text-15 font-semibold flex items-center hover:text-skyAqua`}
    >
      {link.linkText}
    ...
    {link.linkText === 'Services' && (
      <div
        className="absolute left-0 top-[100%] w-40 bg-white p-7 transition-all duration-300 ease-in-out z-[60] max-h-96 opacity-0 invisible group-hover:opacity-100 group-hover:visible group-hover:max-h-[calc(100vh-5rem)] shadow-[0_0_20px_rgba(0,0,0,.15)]"
        ...
        <div className="flex flex-col space-y-5">
          {link?.subLinks?.map((subLink: any, k: number) => (
            <Link
              ...
              key={k}
              className={`${subLink.active ? 'text-skyAqua' : 'text-zinc-800'} font-semibold text-15 hover:text-skyAqua`}
            >
              {subLink.linkText}
            </Link>
          ))}
...
const HeaderBottom = () => {
  ...
  return (
    <div className="bg-white h-20 hidden 990:flex items-center w-full">
