...
import YellowCircleBluePlusSVG from '@/app/icons/YellowCircleBluePlusSVG'
...
const PublicAlbum = ({ id, name, galleryItems, serviceType }: ProjectProps) => {
  ...
  return (
    ...
    >
      <Picture src={galleryItems[0]?.after?.url} className="w-full h-full object-cover" priority={false} />
      <div
        className="group-hover:translate-y-0 transform translate-y-full transition-all duration-300 w-full bg-skyAqua h-0 group-hover:h-full absolute bottom-0 left-0"
        style={{
          clipPath: 'polygon(0% 70%, 100% 50%, 100% 100%, 0% 100%)'
        }}
      />
      <YellowCircleBluePlusSVG className="absolute right-4 top-[44%] opacity-0 group-hover:opacity-100 group-hover:-translate-x-4 transform translate-x-4 transition-all duration-300 hover:rotate-45" />

      <h1 className="text-2xl text-white font-bold absolute bottom-10 left-8 opacity-0 transform transition-all duration-300 translate-y-4 group-hover:opacity-100 group-hover:-translate-y-4 hover:text-sunny">
        {name}
      </h1>
